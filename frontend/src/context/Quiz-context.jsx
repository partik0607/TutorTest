...
const CustomQuizProvider = ({ children }) => {
  ...
  const onStartQuiz = async (input) => {
    const { topic, score, numberOfQuestions, startDate, lastDate, questions } = input;
    // console.log(input);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to create quiz, server responded with status ' + response.status);
      }
  ...
  console.log(users)
  return (
    <quizcontext.Provider value={{ quiz, setquiz,onSubmit ,onStartQuiz,Student,users}}>
      {children}
